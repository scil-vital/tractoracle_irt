...
class VerifyberApptainerRunner():
    def __init__(self, sif_img_path, use_slurm_tmpdir=True):
        self.sif_img_path = sif_img_path
        self.use_slurm_tmpdir = use_slurm_tmpdir
        if not os.path.exists(self.sif_img_path):
            raise FileNotFoundError(f"Apptainer image not found: {self.sif_img_path}")

    def run(self, config_path, output_dir):
        trk_file, t1_file, fa_file = parse_config(config_path)

        with GetTempDir(use_slurm_tmpdir=self.use_slurm_tmpdir) as tmp_dir:
            LOGGER.info("==========================================")
            LOGGER.info(f"Running Verifyber with config: {config_path}")
            LOGGER.info(f" ↳ Container: {self.sif_img_path}")
...
class VerifyberDockerRunner():
    ...
    def run(self, config_path, output_dir):
        ...
        LOGGER.info("==========================================")
        LOGGER.info(f"Running Verifyber with config: {config_path}")
        LOGGER.info(f" ↳ Container: {self.docker_image}")
        LOGGER.info(f" ↳ Config file: {config_path}")
        LOGGER.info(f" ↳ Output directory: {output_dir}")
        LOGGER.info(f" ↳ TRK file: {trk_file}")
        LOGGER.info(f" ↳ T1 file: {t1_file}")
        ...
        command = self._build_command(output_dir, trk_file, t1_file, fa_file)
        os.makedirs(output_dir, exist_ok=True)
    ...
    def _build_command(self, output_dir, trk_file, t1_file, fa_file):
        # Prepare the command to run the Apptainer container.
        command = [
            "docker", "run", "--gpus", "all",
            "-v", f"{output_dir}:/app/output",
            "-v", f"{trk_file}:/app/tractogram.trk:ro",
            "-v", f"{t1_file}:/app/t1.nii.gz:ro",
            "-t", self.docker_image
        ]
        return command
