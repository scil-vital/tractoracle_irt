...
from tractoracle_irt.environments.stopping_criteria import StoppingFlags, count_flags
...
class Tracker(object):
    ...
    def track_mc(
        ...
        tracts_format : TrkFile or TckFile
            Tractogram format.
        stopping_stats: dict
            As this function returns a generator,
            this passed dict will be updated with the
            stopping statistics of the streamlines.
        ...
        def update_stopping_stats(item):
            """
            The stopping stats should hold the following structure:
            stopping_stats = {
                'total_nb_streamlines': 
                'flags': {
                    'flag_1': nb_streamlines,
                    'flag_2': nb_streamlines,
                    ...
                    'flag_n': nb_streamlines,
                },
            }
            """
            if stopping_stats is not None:
                # INIT if not already done                
                if 'total_nb_streamlines' not in stopping_stats:
                    stopping_stats['total_nb_streamlines'] = 0

                if 'flags' not in stopping_stats:
                    stopping_stats['flags'] = {}
                    for f in StoppingFlags:
                        stopping_stats['flags'][f.name] = 0
                    stopping_stats['flags']["STOPPING_TARGET_only"] = 0
                    stopping_stats['flags']["STOPPING_MASK_only"] = 0
                
                # Update the stopping stats
                flags = item.data_for_streamline['flags']
                N = len(flags)

                stopping_stats['total_nb_streamlines'] += N
                for f in StoppingFlags:
                    stopping_stats['flags'][f.name] += count_flags(flags, f)
                
                for f in flags:
                    # Count if only STOPPING_TARGET
                    if f == StoppingFlags.STOPPING_TARGET.value:
                        stopping_stats['flags']["STOPPING_TARGET_only"] += 1

                    # Count if only STOPPING_MASK
                    if f == StoppingFlags.STOPPING_MASK.value:
                        stopping_stats['flags']["STOPPING_MASK_only"] += 1
        ...
        def tracking_generator():
            ...
            for start in tqdm(range(0, len(env.seeds), batch_size), desc="Tracking"):
                ...
                # Track forward
                self.alg.validation_episode(
                    state, env, self.prob, compute_reward=True)
